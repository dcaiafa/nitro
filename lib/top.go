...
func top(m *nitro.Machine, caps []nitro.ValueRef, args []nitro.Value, retN int) ([]nitro.Value, error) {
...
func (i *topIter) Next(m *nitro.Machine, caps []nitro.ValueRef, args []nitro.Value, retN int) ([]nitro.Value, error) {
	if i.nLeft <= 0 {
		res := make([]nitro.Value, retN)
		res[0] = nitro.NewBool(false)
		return res, nil
	}

	v, ok, err := nitro.Next(m, i.inIter, retN-1)
	if err != nil {
		return nil, err
	}
