...
func (s *VarDeclStmt) RunPass(ctx *Context, pass Pass) {
		...
		if s.InitValues != nil {
			...
			for _, sym := range s.syms {
				...
				case *symbol.LocalVarSymbol:
					emitter.Emit(s.Pos(), runtime.OpLoadLocalRef, uint32(sym.LocalNdx), 0)
				case *symbol.GlobalVarSymbol:
					emitter.Emit(s.Pos(), runtime.OpLoadGlobalRef, uint32(sym.GlobalNdx), 0)
				}
			}
		}
	}
	...
	case Emit:
		if s.InitValues != nil {
			emitter := ctx.Emitter()
			emitter.Emit(s.Pos(), runtime.OpStore, uint32(len(s.syms)), 0)
		}
	}
}
